...
async def got_location(m: Message):
    ...
    await m.answer("Геопозиция получена. Выбери, что хочешь сделать сейчас:", reply_markup=intents_kb())
...
async def choose_intent(m: Message):
    ...
    await m.answer("На какой срок включить видимость?", reply_markup=visibility_kb())
